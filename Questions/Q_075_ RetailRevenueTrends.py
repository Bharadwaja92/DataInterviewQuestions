...
The format of the result should look like the following:
...
df = pd.DataFrame(columns=['order_id', 'channel', 'date', 'month', 'revenue'])

gdf = df.groupby(by=['month'])['revenue'].mean()
